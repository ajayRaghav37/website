	...
	var ModuleClassLoader = Spine.Class.create({
		init: function(options){
			...
			*/
			...
			// Add performance labels for module asset burnout, if possible
			try {
				top._performanceLabels = top._performanceLabels || [];
				top._performanceLabels.push(webs.site.consolidatedModuleAssets ? 'moduleAssetsBurnedOut' : 'moduleAssetsAJAXLoad');
			} catch (ex) {
		...
		*/
		...
		createAppModule: function(moduleSlug, options){
			...
			if(!moduleClass) {
				...
				moduleClass = this.classes[moduleSlug] = webs_modules.AppModule.create({id:moduleSlug.substring(12)});
		...
		create: function(moduleSlug, options){
			if(moduleSlug.indexOf('app-sidebar-') === 0) return this.createAppModule(moduleSlug, options);
		...
		*/
		...
		shouldConsolidateAssets: function(moduleSlug) {
			return  webs.site.consolidatedModuleAssets &&
					moduleSlug.indexOf("zumba") == -1 &&
					moduleSlug.indexOf("app-sidebar") == -1;
		},
